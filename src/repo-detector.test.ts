...
import { parseGitUrl, detectRepoType, getRepoDisplayName } from './repo-detector.js';

describe('detectRepoType', () => {
  test('detects GitHub SSH URL', () => {
    assert.strictEqual(detectRepoType('git@github.com:owner/repo.git'), 'github');
  });

  test('detects GitHub HTTPS URL', () => {
    assert.strictEqual(detectRepoType('https://github.com/owner/repo.git'), 'github');
  });

  test('detects Azure DevOps SSH URL', () => {
    assert.strictEqual(detectRepoType('git@ssh.dev.azure.com:v3/org/project/repo'), 'azure-devops');
  });

  test('detects Azure DevOps HTTPS URL', () => {
    assert.strictEqual(detectRepoType('https://dev.azure.com/org/project/_git/repo'), 'azure-devops');
  });

  test('defaults to GitHub for unknown URLs', () => {
    assert.strictEqual(detectRepoType('git@gitlab.com:owner/repo.git'), 'github');
  });
});
...
describe('parseGitUrl', () => {
  ...
  describe('Azure DevOps URLs', () => {
    test('parses SSH format: git@ssh.dev.azure.com:v3/org/project/repo', () => {
      const result = parseGitUrl('git@ssh.dev.azure.com:v3/myorg/myproject/myrepo');
    ...
    test('parses HTTPS format: https://dev.azure.com/org/project/_git/repo', () => {
      const result = parseGitUrl('https://dev.azure.com/myorg/myproject/_git/myrepo');
      assert.strictEqual(result.type, 'azure-devops');
      assert.strictEqual(result.organization, 'myorg');
      assert.strictEqual(result.project, 'myproject');
      assert.strictEqual(result.repo, 'myrepo');
    ...
    test('handles repo names with dots in Azure DevOps', () => {
      const result = parseGitUrl('git@ssh.dev.azure.com:v3/org/project/my.repo.name');
      assert.strictEqual(result.repo, 'my.repo.name');
    });

    test('handles repo names with dots and .git suffix in Azure DevOps', () => {
      const result = parseGitUrl('https://dev.azure.com/org/project/_git/my.repo.git');
      assert.strictEqual(result.repo, 'my.repo');
    });

    test('handles project names with hyphens', () => {
      const result = parseGitUrl('https://dev.azure.com/org/my-project/_git/repo');
